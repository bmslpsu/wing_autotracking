...
for ii = 1:num_vid
    filepath = utils.select_tif_folder(tifRootPath, ii, 'Comp Uint8 Mask Tifs');
...
for ii = 1:num_vid
    ...
    saveTiffPath = fullfile(tifRootPath, 'wing_mask_Uint8', sep_path{end});
    if ~isfolder(saveTiffPath)
        mkdir(saveTiffPath)
    end

    % get body tif saving path
    sep_path = strsplit(pathVector{ii},filesep);
