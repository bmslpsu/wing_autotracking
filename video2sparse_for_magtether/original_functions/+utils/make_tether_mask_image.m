function [] = make_tether_mask_image(Vreader, frame_window, sI, eI, bottomFlag, ...
    tether_pts, background, savedir)
%% Generates the tether and body mask while subtracting the background
% INPUTS:
%   Vreader     : video read by VideoReader
%   frame_window: # of frames to take before and after current frame for buffer
...
%% Generate Masks
disp('Generating masks for tether')

tetherpath = fullfile(savedir, 'frame_');
...
for n = 1:n_frame % each frame
    ...
    mask_tether = utils.seperate_tether(mask_body_tether, tether_pts, bottomFlag); % seperate body & tether masks
    
    % Clean masks
    mask_tether = process_mask(mask_tether, []);
    
    % Save the current mask for body & tether
    imwrite(mask_tether, [tetherpath num2str(frames(n)) '.' image_type], image_type)
