    ...
    methods
        ...
        function [hull_recon,ind0_frame] = hull_reconstruction(obj,field_string,createvol,ind0_frame,varargin)
            ...
            for kcam=camvec(2:end)
                ...
                % mirror)
                if kcam == obj.cameras.all.ZaxCam && obj.cameras.all.ZaxCam ~=0
                    obj.cameras.all.size_image(1)
                    im1Coords = [im1Coords(:,1) (obj.cameras.all.size_image(1)+1) - im1Coords(:,2)];
                    % m1Coords = [im1Coords(:,1) im1Coords(:,2)];
                    % im1Coords = [im1Coords(:,1) 513 - im1Coords(:,2)];
            ...
            if createvol == 0
                ...
                hull_recon=unique(checkifall_cam_ind(maph == length(obj.cameras.all.camvec),:),'rows');
            elseif createvol == 2
                ind0_frame=unique(checkifall_cam_ind(maph == length(obj.cameras.all.camvec),:),'rows')';
            else
                ...
                % frame
                hull_recon_real=unique(checkifall_cam_real(maph == length(obj.cameras.all.camvec),:),'rows');
