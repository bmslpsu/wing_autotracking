...
%                     end
        ...
        function wingAngles(obj,varargin)
            ...
            for k = 1:1:2
                ...
                % constrain for phi(stroke), because the phi may reach 300
                % degrees in Wael's data, which is not possible. By Yue
                % Zhu.
                temp_phi = phitmp'*180/pi;
                for i = 1:length(temp_phi)
                    single_temp_phi = temp_phi(i);
                    % if single_temp_phi >= 0 && single_temp_phi <= 180
                    %     temp_phi(i) = single_temp_phi;
                    if single_temp_phi <= 180
                        temp_phi(i) = single_temp_phi;
                    % elseif single_temp_phi <=0
                    %     temp_phi(i) = abs(single_temp_phi);
                    elseif single_temp_phi >= 180
                        single_temp_phi = 180-abs(single_temp_phi-180);
                        temp_phi(i) = abs(single_temp_phi);
                    end
                end
                obj.(wingname{k}).angles.phi = temp_phi;
